  ...
  describe('enrollFree', () => {
    ...
    it('should throw ConflictException if already enrolled', async () => {
      ...
      it('should throw NotFoundException if course does not exist', async () => {
        ...
        await expect(service.enrollFree(studentId, courseId)).rejects.toThrow(NotFoundException);
      });
    ...
    describe('checkoutCart', () => {
      it('should throw BadRequestException if cart is empty', async () => {
        const studentId = 'student1';
        cartItemModel.find.mockReturnValue({ exec: jest.fn().mockResolvedValue([]) });
        await expect(service.checkoutCart(studentId)).rejects.toThrow(BadRequestException);
      });

      it('should skip already enrolled courses', async () => {
        const studentId = 'student1';
        const cartItems = [{ _id: 'cart1', courseId: 'course1' }];
        cartItemModel.find.mockReturnValue({ exec: jest.fn().mockResolvedValue(cartItems) });
        const mockCourse = { _id: 'course1', price: 0, status: 'published' };
        courseModel.findById.mockReturnValue({ exec: jest.fn().mockResolvedValue(mockCourse) });
        enrollmentModel.findOne.mockReturnValue({ exec: jest.fn().mockResolvedValue({ _id: 'enrollment1' }) });
        cartItemModel.findByIdAndDelete.mockReturnValue({ exec: jest.fn().mockResolvedValue({}) });
      ...
      it('should enroll free courses in cart', async () => {
        ...
        cartItemModel.find.mockReturnValue({ exec: jest.fn().mockResolvedValue(cartItems) });
        const mockCourse = { _id: 'course1', price: 0, status: 'published', tutorId: 'tutor1', tutorEmail: 'tutor@email.com' };
        courseModel.findById.mockReturnValue({ exec: jest.fn().mockResolvedValue(mockCourse) });
        // Patch the Enrollment model constructor directly on the service instance
        const enrollmentSaveMock = jest.fn().mockResolvedValue({ studentId, courseId: 'course1', type: 'free', amountPaid: 0, status: 'completed' });
        ...
        enrollmentModel.findOne.mockImplementation(() => ({ exec: jest.fn().mockResolvedValue(null) }));
        courseModel.findByIdAndUpdate.mockReturnValue({ exec: jest.fn().mockResolvedValue(mockCourse) });
        cartItemModel.findByIdAndDelete.mockReturnValue({ exec: jest.fn().mockResolvedValue({}) });
        const result = await service.checkoutCart(studentId);
    ...
    describe('getMyCourses', () => {
      it('should return empty array if no enrollments', async () => {
        const studentId = 'student1';
        enrollmentModel.find.mockReturnValue({ exec: jest.fn().mockResolvedValue([]) });
      ...
      it('should return enrollment and course pairs', async () => {
        ...
        const courses = [{ id: 'course1', title: 'Course 1', description: 'desc', thumbnailUrl: null, tutorName: 'Tutor' }];
        enrollmentModel.find.mockReturnValue({ exec: jest.fn().mockResolvedValue(enrollments) });
        // Ensure courseModel is available
        if (!courseModel.find) courseModel.find = jest.fn();
        courseModel.find.mockReturnValue({ exec: jest.fn().mockResolvedValue(courses) });
